...
# pip install transformers torch rouge-score bert-score sacrebleu
...
def generate_summaries_bart(model, tokenizer, inputs):
    summaries = []
    for inp in tqdm(inputs, desc=f"Generating summaries with {MODEL_NAME}"):
        input_ids = tokenizer.encode(inp, return_tensors='pt', truncation=True, max_length=1024)
...
if __name__ == "__main__":
    ...
    val_hyps = generate_summaries_bart(model, tokenizer, validation_inputs)
    ...
    test_hyps = generate_summaries_bart(model, tokenizer, test_inputs)
